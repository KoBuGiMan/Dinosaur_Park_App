...
class Ticket {
	...
	public Ticket() {
		...
		year = allDay.getYear();
		day = allDay.getDayOfMonth();
		month = allDay.getMonthValue();
...
//------------------------------------------------------------------------------------------------------------------
	...
	public void openPark(Park park) {
		System.out.println("공원개방시간을 조회하시겠습니까? (네/아니요)");
		String ans = sc.next();
		System.out.println();
		if (ans.equals("네")) {
			...
			if (ans.equals("1동")) {
				...
				for (int i = 0; i < dino.copyDinoList.length; i++) {
					if (dino.copyDinoList[i][3].equals("1동")) {
						System.out.println(Arrays.deepToString(dino.copyDinoList[i]));
						continue;
					}
				}
	...
	public void foodCheck(Park park, Dino dino) {
		...
		if (park.openClose.equals("open")) {
			...
			for (int i = 0; i < dino.copyDinoList.length; i++) {
				if (ans.equals(dino.copyDinoList[i][0])) {
					...
					System.out.println("현재 저희 공원에는 존재하지 않는 종입니다.");
				}
			}
	...
	public String[] getFood() {
		return new String[] { food.spe, food.name, food.value, food.value, food.weight, food.gram, food.cost };
	}

	// ---------------------------------------------------------------------------------------------------

	public void allTicket(Ticket ticket, Park park) {

		if (park.openClose.equals("open")) {
			String ans = null;
			do {
				System.out.println("//////////////////////////////////");
				System.out.println("<<<<<<공원입장권 가격>>>>>>");
				System.out.println("아동(0세~7세): 5000원");
				System.out.println("청소년 및 성인(8세~60세): 10000원");
				System.out.println("노인(60세~ ): 7000원");
				System.out.println("//////////////////////////////////");
				System.out.println();
				System.out.println("개표자의 이름을 입력해 주세요:");
				String name = sc.next();
				System.out.println("각각 인원이 몇명인지 입력해 주세요:");
				System.out.println("아동(0세~7세): ");
				int ageChild = sc.nextInt();
				System.out.println("청소년 및 성인(8세~60세):");
				int ageAdult = sc.nextInt();
				System.out.println("노인(60세~ ):");
				int ageOld = sc.nextInt();

				System.out.println("아동 " + ageChild + "명 / " + "청소년 및 성인 " + ageAdult + "명 / " + "노인 " + ageOld + "명");
				System.out.println(
						"가격: " + ticket.priceChild + "원 X " + ageChild + "명 = " + ticket.priceChild * ageChild + "원");
				System.out.println(
						"     " + ticket.priceAdult + "원 X " + ageAdult + "명 = " + ticket.priceAdult * ageAdult + "원");
				System.out
						.println("     " + ticket.priceOld + "원 X " + ageOld + "명 = " + ticket.priceOld * ageOld + "원");
				System.out.println("총액: "
						+ ((ticket.priceChild * ageChild) + (ticket.priceAdult * ageAdult) + (ticket.priceOld * ageOld))
						+ "원");
				System.out.println("인원이 맞는지 확인해 주세요 (네/아니요)");
				String ans1 = sc.next();

				if (ans1.equals("네")) {
					System.out.println(
							"/////////////////////////////////////////////////////////////////////////////////////////");
					System.out.println("계표자명: " + name);
					System.out.println("날짜: " + ticket.year + "년" + ticket.month + "월" + ticket.day + "일");
					System.out.println("인원: " + "아동 " + ageChild + "명 / " + "청소년 및 성인 " + ageAdult + "명 / " + "노인 "
							+ ageOld + "명");
					System.out.println("총액: " + ((ticket.priceChild * ageChild) + (ticket.priceAdult * ageAdult)
							+ (ticket.priceOld * ageOld)) + "원");
					System.out.println(
							"/////////////////////////////////////////////////////////////////////////////////////////");
					return;
				} else if (ans1.equals("아니요")) {
					System.out.println("인원을 다시 설정하시겠습니까? (네/아니요)");
					ans = sc.next();
				} else {
					System.out.println("개표시스템을 종료합니다.");
					return;
				}
			} while (ans.equals("네"));
	...
	public static void main(String[] args) {
		...
		System.out.println(ticket.year);
		System.out.println(ticket.month);
		System.out.println(ticket.day);
		String an = "네";
		do {
			...
			System.out.println("7. 에리어별 공룡조회");
			System.out.println("8. 식성등록");
			...
			if (num == 1) {
				...
				di.allTicket(ticket, park);
			} else if (num == 7) {
				di.Enclo(dino, park);
			} else if (num == 8) {
				di.foodCheck(park, dino);
			} else if (num == 0) {
				an = "아니요";
